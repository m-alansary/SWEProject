...
int main(int argc, char *argv[]){
    ...
    SQLiteDb.sql_create(table, columns, types);

    SQLiteDb.sql_insert(table, columns2, values);
    SQLiteDb.sql_insert(table, columns2, values);
    SQLiteDb.sql_insert(table, columns2, values);
    SQLiteDb.sql_insert(table, columns2, values);

    SQLiteDb.sql_update(table, columns2, values2, "ID = 1");

    SQLiteDb.sql_delete(table, "ID = 1");

    SQLiteDb.sql_select("*", table, "");
    SQLiteDb.sql_print();
